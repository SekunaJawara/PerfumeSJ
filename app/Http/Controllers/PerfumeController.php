...
class PerfumeController extends Controller
{
    ...
    public function index()
    {
        return view(
            'perfumes.index',
            [
                'perfumes' => Perfume::latest()->filter(request(['nota', 'search']))->take(6)->get()

            ]
        );
    }
    
    //Muestra todos los perfumes sin límite
public function showAll(){
    return view('perfumes.all',[
        'perfumes' => Perfume::latest()->filter(request(['nota','search']))->get()
    ]);
}
