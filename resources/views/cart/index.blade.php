          ...
          <tr class="text-center">
            ...
            >
              ...
              <svg
                width="14"
                height="15"
                class="cursor-pointer btn-minus"
                data-id="{{ $item->id }}"
              ...
              <span
                id="qty-{{ $item->id }}"
                class="w-10 text-center text-[#191919] text-base font-normal leading-normal"
              >{{ $item->quantity }}</span>
              <!-- Botón sumar -->
              <svg
                class="cursor-pointer relative btn-plus"
                data-id="{{ $item->id }}"
                data-url="{{ route('cart.update', $item->id) }}"
                width="14"
                height="15"
    ...
    <div class="w-[424px] bg-white rounded-lg p-6">
      ...
      <div class="w-[376px] py-3 justify-between items-center flex">
        <span class="text-[#4c4c4c] text-base font-normal leading-normal"
          >Total:</span
        ><span id="cart-total" class="text-[#191919] text-base font-semibold leading-tight"
          >{{ number_format($cartItems->sum(fn($i) => $i->perfume->price * $i->quantity), 2) }}€</span
        >
      </div>
      <div
      ...
      >
        <span class="text-[#4c4c4c] text-sm font-normal leading-[21px]"
          >Subtotal:</span
          ><span id="cart-subtotal" class="text-[#191919] text-sm font-medium leading-[21px]"
          >{{ number_format($cartItems->sum(fn($i) => $i->perfume->price * $i->quantity), 2) }}€</span
          >
      </div>
      <button type="submit"
